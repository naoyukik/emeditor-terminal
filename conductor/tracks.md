# Project Tracks

This file tracks all major tracks for the project. Each track has its own detailed plan in its respective folder.

---

- [x] **Track: EmEditorのカスタムバー機能を活用し、Windows Pseudo Console (ConPTY) をバックエンドとしたターミナル画面を表示する**
*Link: [./archive/custom_window_conpty_20260117/](./archive/custom_window_conpty_20260117/)*

---

- [x] **Track: feat: 追加のカーソル移動シーケンス (CHA, VPA, CNL, CPL) のサポート (Issue #11)**
*Link: [./archive/11_cursor_movement_20260123/](./archive/11_cursor_movement_20260123/)*

---

- [x] **Track: feat: カーソル位置の保存/復元 (DECSC/DECRC) の実装 (Issue #12)**
*Link: [./archive/12_cursor_save_restore_20260124/](./archive/12_cursor_save_restore_20260124/)*

---

- [x] **Track: fix: PSReadLineの予測入力（Prediction）表示の改善**
*Link: [./archive/19_fix_psreadline_prediction_20260125/](./archive/19_fix_psreadline_prediction_20260125/)*

---

- [x] **Track: スクロールバック・バッファ（履歴バッファ）の実装**
*Link: [./archive/23_scrollback_buffer/](./archive/23_scrollback_buffer/)*

---

- [x] **Track: PR #54 Review Fixes (Guidelines)**
*Link: [./archive/pr_review_54_guidelines_20260130/](./archive/pr_review_54_guidelines_20260130/)*

---

- [x] **Track: PR #55 Review Fixes: Improve input logic safety, coverage, and maintainability**
*Link: [./archive/pr_review_55_refactor_input/](./archive/pr_review_55_refactor_input/)*

---

- [x] **Track: PR #57 Review Fixes**
*Link: [./archive/pr_review_57_fixes/](./archive/pr_review_57_fixes/)*

---

- [x] **Track: PR #59 Review Fixes: Format and Style**
*Link: [./archive/59_pr_review_fixes_20260202/](./archive/59_pr_review_fixes_20260202/)*

---

- [~] **Track: fix: PR #62 Review Fixes (RegisterClassW, Handle Safety, Comments)**
*Link: [./tracks/64_pr_review_fixes/](./tracks/64_pr_review_fixes/)*



