...
- [ ] Task: Conductor - User Manual Verification 'フェーズ 1'
...
- [ ] Task: `TerminalBufferEntity::get_selected_text()` の既存挙動を固定する単体テストを追加または更新する
    - [ ] `src/domain/model/terminal_buffer_entity.rs` の末尾にテストを追加。
    - [ ] テスト内容: 単一行選択、複数行選択、空の選択、ワイド文字（継続セルスキップ）の検証。
- [ ] Task: 変更前テストを実行し、現行挙動を正しく表現していることを確認する
- [ ] Task: Conductor - Clippy & fmt Check
- [ ] Task: Conductor - User Manual Verification 'フェーズ 2'
- [ ] Task: Conductor - 'フェーズ 2' の成果をコミット

## フェーズ 3: `get_selected_text()` の効率化実装
- [ ] Task: `get_selected_text()` の抽出範囲算出を行単位へ変更する
    - `src/domain/model/terminal_buffer_entity.rs` の `get_selected_text` を修正。
- [ ] Task: Conductor - Clippy & fmt Check
- [ ] Task: Conductor - User Manual Verification 'フェーズ 3'
- [ ] Task: Conductor - 'フェーズ 3' の成果をコミット

## フェーズ 4: 総合検証と完了処理
- [ ] Task: 全体テストを実行する (`cargo test`)
