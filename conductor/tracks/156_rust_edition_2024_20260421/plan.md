...
- [ ] Task: `src/gui/common/mod.rs` 内の `pixels_to_points`, `points_to_pixels` 関連の `unsafe fn` ボディを `unsafe {}` ブロック化し、Safety Comment を付与する
- [ ] Task: `src/gui/driver/config_gui_driver.rs` 内の `update_font_label`, `settings_dlg_proc` 等の `unsafe fn` ボディを `unsafe {}` ブロック化し、Safety Comment を付与する
- [ ] Task: エディション移行に必須な最小限のクレート（依存関係）の更新 (`cargo update`)
...
- [ ] Task: `src/gui/driver/scroll_gui_driver.rs:19` の `extern` ブロックを `unsafe extern` に変更
- [ ] Task: `src/lib.rs` 内の `#[no_mangle]` 属性を `#[unsafe(no_mangle)]` に変更
