...
- [x] Task: Conductor - User Manual Verification 'Phase 1 - 設定構造の定義' (Protocol in workflow.md)
...
- [x] Task: テストコード（Mock を用いた Config 取得テスト）の作成。
- [x] Task: Commit Phase 2 - `feat: EmEditor標準APIによる設定読み書きの実装`
- [x] Task: Conductor - User Manual Verification 'Phase 2 - インフラ層の読み書き' (Protocol in workflow.md)

## Phase 3: アプリケーション層への統合 (Application Integration)
**目的:** 起動時に設定をロードし、サービスへ注入する。

- [x] Task: `TerminalWorkflow` および `TerminalService` のコンストラクタで `ConfigRepository` を受け取るように修正。
- [x] Task: `TerminalWorkflow` の初期化プロセスで `config_repo.load()` を実行し、設定を適用する。
