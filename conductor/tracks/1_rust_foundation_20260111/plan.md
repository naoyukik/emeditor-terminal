...
- [x] タスク: Conductor - ユーザー手動検証 'プロジェクトの初期化' (workflow.md のプロトコルに従う)

## フェーズ 2: EmEditor SDK 統合 (FFI)
- [x] タスク: EmEditor FFI 型の定義
  - [x] EmEditor SDK ヘッダーに対応する Rust の構造体/型を作成する（`LOAD_INFO`, `ET_COMMAND` など）。
...
- [x] タスク: Hello World ロジックの実装
  - [x] 実行を確認するために、`OnCommand` または `DllMain` (ProcessAttach) に `MessageBoxW` の呼び出しを追加する。
