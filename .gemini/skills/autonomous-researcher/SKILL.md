...
- `google_web_search` や `microsoft_docs_search` を使用し、対象となるAPI、ライブラリ、またはプロトコルの最新仕様を調査する。
- **EmEditor SDK 固有の調査**を行う場合は、まず `references/emeditor_sdk.md` を読み込み、既知のメッセージIDや罠を確認せよ。
- 関連するURLを特定したら、`web_fetch` や `microsoft_docs_fetch` で内容を詳細に解析する。
...
- 例：Win32 API の直接呼び出しは `Infrastructure` レイヤーに閉じ込めているか、`RAII` パターンに従っているか。
