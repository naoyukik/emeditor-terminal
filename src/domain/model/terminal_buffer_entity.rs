...
impl TerminalBufferEntity {
    ...
    fn get_empty_cell(&self) -> Cell {
        Cell { text: " ".to_string(), attribute: TerminalAttribute { fg: TerminalColor::Default, bg: self.current_attribute.bg.clone(), ..TerminalAttribute::default() }, is_wide_continuation: false }
    }
    ...
    pub fn print_cell(&mut self, c: char) {
        ...
        if clusters.len() > 1 {
            ...
            for cluster in clusters {
                ...
                if self.cursor.x + w > self.width { self.cursor.x = 0; self.index(); }
                self.grid.put_cell(self.cursor.x, self.cursor.y, Cell { text: cluster, attribute: self.current_attribute.clone(), is_wide_continuation: false }, w, &self.current_attribute.bg);
                self.cursor.x += w;
            }
    ...
    pub fn scroll_up(&mut self) {
        ...
        if self.scroll_top == 0 && self.scroll_bottom == self.height.saturating_sub(1) {
            if let Some(line) = self.grid.lines_mut().pop_front() { self.scrollback.push(line); }
            self.grid.lines_mut().push_back(vec![empty; self.width]);
        } else {
            self.grid.delete_lines(self.scroll_top, 1, self.scroll_bottom, vec![empty; self.width]);
        }
    }

    pub fn scroll_down(&mut self) {
        let empty = self.get_empty_cell();
        self.grid.insert_lines(self.scroll_top, 1, self.scroll_bottom, vec![empty; self.width]);
    }
    ...
    pub fn move_cursor_to_pos(&mut self, r: usize, c: usize) {
        let y = if self.is_origin_mode { (self.scroll_top + r).saturating_sub(1) } else { r.saturating_sub(1) };
    ...
    pub fn move_cursor_backward(&mut self, n: usize) {
        for _ in 0..n {
            ...
            self.cursor.x -= 1;
            if self.grid.lines()[self.cursor.y][self.cursor.x].is_wide_continuation && self.cursor.x > 0 { self.cursor.x -= 1; }
        }
    }

    pub fn handle_tab(&mut self) {
        let nx = (self.cursor.x / 8 + 1) * 8;
        if nx >= self.width { self.cursor.x = 0; self.index(); }
        else { while self.cursor.x < nx { let empty = self.get_empty_cell(); self.grid.put_cell(self.cursor.x, self.cursor.y, empty, 1, &self.current_attribute.bg); self.cursor.x += 1; } }
    }
    ...
    pub fn insert_lines(&mut self, n: usize) {
        ...
        let empty = self.get_empty_cell();
        self.grid.insert_lines(self.cursor.y, n, self.scroll_bottom, vec![empty; self.width]);
    }
    ...
    pub fn delete_lines(&mut self, n: usize) {
        ...
        let n = n.min(self.scroll_bottom - self.cursor.y + 1);
        let empty = self.get_empty_cell();
        self.grid.delete_lines(self.cursor.y, n, self.scroll_bottom, vec![empty; self.width]);
    }

    pub fn insert_cells(&mut self, n: usize) {
        let n = n.min(self.width - self.cursor.x);
        if n > 0 {
            self.grid.ensure_safe_boundary(self.cursor.y, self.cursor.x, &self.current_attribute.bg);
            let empty = self.get_empty_cell();
            let line = &mut self.grid.lines_mut()[self.cursor.y];
            for _ in 0..n { line.insert(self.cursor.x, empty.clone()); line.pop(); }
        }
    }
    ...
    pub fn delete_cells(&mut self, n: usize) {
        ...
        if n == 0 { return; }
        self.grid.ensure_safe_boundary(self.cursor.y, self.cursor.x, &self.current_attribute.bg);
        self.grid.ensure_safe_boundary(self.cursor.y, (self.cursor.x + n).min(self.width - 1), &self.current_attribute.bg);
        let empty = self.get_empty_cell();
        let line = &mut self.grid.lines_mut()[self.cursor.y];
    ...
    pub fn erase_cells(&mut self, n: usize) { let empty = self.get_empty_cell(); self.grid.fill_line(self.cursor.y, self.cursor.x, self.cursor.x + n, empty); }
    pub fn erase_in_line(&mut self, mode: u8) {
        let empty = self.get_empty_cell();
        match mode {
            0 => self.grid.fill_line(self.cursor.y, self.cursor.x, self.width, empty),
            1 => self.grid.fill_line(self.cursor.y, 0, self.cursor.x + 1, empty),
    ...
    pub fn erase_in_display(&mut self, mode: u8) {
        let empty = self.get_empty_cell();
        match mode {
            0 => { self.erase_in_line(0); for y in (self.cursor.y + 1)..self.height { self.grid.fill_line(y, 0, self.width, empty.clone()); } }
            1 => { for y in 0..self.cursor.y { self.grid.fill_line(y, 0, self.width, empty.clone()); } self.erase_in_line(1); }
            2 | 3 => { for y in 0..self.height { self.grid.fill_line(y, 0, self.width, empty.clone()); } }
            _ => {}
        }
    }
    ...
    pub fn set_origin_mode(&mut self, on: bool) { self.is_origin_mode = on; self.cursor.y = if on { self.scroll_top } else { 0 }; self.cursor.x = 0; }
    pub fn set_cursor_visible(&mut self, v: bool) { self.cursor.is_visible = v; }
    pub fn set_cursor_style(&mut self, s: CursorStyle) { self.cursor.style = s; }
    pub fn set_attribute(&mut self, a: TerminalAttribute) { self.current_attribute = a; }
    ...
    pub fn restore_cursor(&mut self) { if let Some((x, y)) = self.saved_cursor { self.cursor.y = y.min(self.height.saturating_sub(1)); self.cursor.x = x.min(self.width.saturating_sub(1)); } }
    pub fn resize(&mut self, w: usize, h: usize) {
        self.grid.resize(w, h);
        self.width = w; self.height = h;
        self.scroll_top = 0; self.scroll_bottom = h.saturating_sub(1);
    ...
    pub fn get_line_at_visual_row(&self, visual_row: usize) -> Option<&Vec<Cell>> {
        let dist = (self.height.saturating_sub(1).saturating_sub(visual_row)) + self.scrollback.viewport_offset();
        if dist < self.grid.lines().len() { self.grid.lines().get(self.grid.lines().len().saturating_sub(1).saturating_sub(dist)) }
        else { self.scrollback.history().get(self.scrollback.history().len().saturating_sub(1).saturating_sub(dist.saturating_sub(self.grid.lines().len()))) }
    }

    pub fn get_width(&self) -> usize { self.width }
    pub fn get_height(&self) -> usize { self.height }
    pub fn get_history_len(&self) -> usize { self.scrollback.history().len() }
    pub fn get_viewport_offset(&self) -> usize { self.scrollback.viewport_offset() }
    ...
    pub fn scroll_to(&mut self, o: usize) { self.scrollback.scroll_to(o); }
    pub fn scroll_lines(&mut self, d: isize) { self.scrollback.scroll_lines(d); }
    pub fn reset_viewport(&mut self) { self.scrollback.reset_viewport(); }
}
