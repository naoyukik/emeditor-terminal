...
fn on_vscroll(hwnd: HWND, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    let data_arc = get_terminal_data();
    let action = {
        let mut data = data_arc.lock().unwrap();
...
fn on_mousewheel(hwnd: HWND, wparam: WPARAM) -> LRESULT {
    let data_arc = get_terminal_data();
    let action = {
        let data = data_arc.lock().unwrap();
...
fn on_lbuttondown(hwnd: HWND) -> LRESULT {
...
fn on_set_focus(hwnd: HWND) -> LRESULT {
...
fn on_keydown(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
...
fn on_syskeydown(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    let vk_code = wparam.0 as u16;
...
fn on_syskeyup(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    let vk_code = wparam.0 as u16;
...
fn on_keyup(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    let vk_code = wparam.0 as u16;
    if vk_code == VK_MENU.0 {
...
fn on_syschar(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
...
fn on_syscommand(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
...
fn on_size(hwnd: HWND, lparam: LPARAM) -> LRESULT {
...
fn on_ime_set_context(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    log::debug!(
...
fn on_ime_start_composition(hwnd: HWND) -> LRESULT {
    {
        let data_arc = get_terminal_data();
        let mut data = data_arc.lock().unwrap();
...
fn on_ime_composition(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    let handled = {
        let data_arc = get_terminal_data();
        let mut data = data_arc.lock().unwrap();
...
fn on_ime_end_composition(hwnd: HWND) -> LRESULT {
    {
        let data_arc = get_terminal_data();
        let mut data = data_arc.lock().unwrap();
...
extern "system" fn wnd_proc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    match msg {
        WM_VSCROLL => on_vscroll(hwnd, wparam, lparam),
        WM_MOUSEWHEEL => on_mousewheel(hwnd, wparam),
        WM_PAINT => on_paint(hwnd),
        WM_LBUTTONDOWN => on_lbuttondown(hwnd),
        WM_SETFOCUS => on_set_focus(hwnd),
        WM_KILLFOCUS => on_kill_focus(),
        WM_KEYDOWN => on_keydown(hwnd, msg, wparam, lparam),
        WM_SYSKEYDOWN => on_syskeydown(hwnd, msg, wparam, lparam),
        WM_SYSKEYUP => on_syskeyup(hwnd, msg, wparam, lparam),
        WM_KEYUP => on_keyup(hwnd, msg, wparam, lparam),
        WM_SYSCHAR => on_syschar(hwnd, msg, wparam, lparam),
        WM_SYSCOMMAND => on_syscommand(hwnd, msg, wparam, lparam),
        WM_GETDLGCODE => {
        ...
        WM_IME_SETCONTEXT => on_ime_set_context(hwnd, msg, wparam, lparam),
        WM_IME_STARTCOMPOSITION => on_ime_start_composition(hwnd),
        WM_IME_COMPOSITION => on_ime_composition(hwnd, msg, wparam, lparam),
        WM_IME_ENDCOMPOSITION => on_ime_end_composition(hwnd),
